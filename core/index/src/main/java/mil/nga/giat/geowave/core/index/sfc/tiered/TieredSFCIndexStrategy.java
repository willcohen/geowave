		...
		HierarchicalNumericIndexStrategy
{
	...
	public List<ByteArrayId> reprojectToCoarserTier(final ByteArrayId insertionId, final byte reprojectTier) {
		...
		if(rowId[0] > reprojectTier) {
			final BinnedNumericDataset[] ranges = BinnedNumericDataset.applyBins(
					initialRange,
					baseDefinitions);
			final Integer reprojectSfcIndex = orderedSfcIndexToTierId.inverse().get(reprojectTier);
			final SpaceFillingCurve reprojectSFC = orderedSfcs[reprojectSfcIndex];
			final List<ByteArrayId> rowIds = new ArrayList<ByteArrayId>(
					ranges.length);
			for( BinnedNumericDataset range : ranges) {
				List<ByteArrayId> ids = TieredSFCIndexStrategy.getRowIdsAtTier(range, reprojectTier, reprojectSFC, BigInteger.valueOf(maxEstimatedDuplicateIdsPerDimension), reprojectSfcIndex);
				rowIds.addAll(ids);
			}
			return rowIds;
		} else {
