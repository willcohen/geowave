		...
		SecondaryIndexDataAdapter<SimpleFeature>
{
	...
	protected Object defaultTypeDataFromBinary(
		...
		//TODO: DONTMAINTAIN! Don't maintain after 1.0. This was specifically made to support GeoWave 0.9.6 - 0.9.7 data
		final byte[] indexCrsBytes;
		...
		if(versionId < VERSION) {
			secondaryIndexBytes = new byte[bytes.length - axisBytes.length - typeNameBytes.length - indexCrsBytes.length
			              				 - namespaceBytes.length - attrBytes.length - encodedTypeBytes.length - 21];
		} else {
			secondaryIndexBytes = new byte[bytes.length - axisBytes.length - typeNameBytes.length - indexCrsBytes.length
			              				 - namespaceBytes.length - attrBytes.length - encodedTypeBytes.length - 25];
		}
