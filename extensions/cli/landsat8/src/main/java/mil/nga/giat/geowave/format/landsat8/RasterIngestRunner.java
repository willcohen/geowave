		...
		DownloadRunner
{
	...
	protected BandData getBandData(
		...
		for (final AttributeDescriptor attr : type.getAttributeDescriptors()) {
			...
			final Object attrValue = band.getAttribute(
					attrName);
			if (attrValue != null) {
				model.put(
						attrName,
		...
		if (ingestOptions.isCropToSpatialConstraint()) {
			...
			if (filter != null) {
				...
				if (geometry != null) {
					...
					final Geometry sceneShape = (Geometry) band.getAttribute(
							SceneFeatureIterator.SHAPE_ATTRIBUTE_NAME);
	...
	protected void nextBand(
		...
		if (ingestOptions.isCoveragePerBand()) {
			...
			try {
				...
				IndexWriter writer = writerCache.get(
						coverageName);
		...
		if (!ingestOptions.isSkipMerge()) {
			...
			for (final PrimaryIndex index : indices) {
				if (dataStorePluginOptions.createDataStoreOperations().mergeData(
					...
					System.out.println(
							"Successfully merged overlapping tiles within index '" + index.getId().getString() + "'");
				}
				else {
					System.err.println(
							"Unable to merge overlapping landsat8 tiles in index '" + index.getId().getString() + "'");
				}
			}
		}
	}
			...
			if (!lastSceneBands.isEmpty()) {
				...
				if (bandsIngested == null) {
					...
					try {
						...
						thisSceneBands)) {
					LOGGER.warn(
							"The bands in this scene ('" + Arrays.toString(
									thisSceneBands) + "') differ from the previous scene ('"
									+ Arrays.toString(
											bandsIngested)
	...
	private synchronized Map<String, Landsat8BandConverterSpi> getRegisteredConverters() {
		if (registeredBandConverters == null) {
			...
			final ServiceLoader<Landsat8BandConverterSpi> converters = ServiceLoader.load(
